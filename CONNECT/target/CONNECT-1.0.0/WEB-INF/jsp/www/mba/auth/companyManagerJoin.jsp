    ...
    <form id="send-form">
		...
		<div class="form-row"> 
		    <div class="form-group col-md-6">
		        ...
		        <input type="text" class="form-control" id="cstmrNm" name="cstmrNm" placeholder="업체명" readonly>
		        <button type="button" class="btn btn-primary" onClick="openSelectCompanyModal()">검색</button>  
		    </div>  
		    <div class="form-group col-md-6">
		        <label for="inputPassword4">사업자 등록번호</label>
		        <input type="text" class="form-control" id="bizrno" name="bizrno" placeholder="사업자등록번호" readonly>
		    </div>
		</div>
		...
		<div class="form-row">
		    <div class="form-group col-md-6">
		        ...
		        <input type="text" class="form-control" id="rprsntvNm" name="rprsntvNm" placeholder="대표자명" readonly>
		    </div>
		    <div class="form-group col-md-6">
		        <label for="inputEmail4">연락처(대표번호)</label>
		        <input type="text" class="form-control" id="telno" name="telno" placeholder="대표 전화번호" readonly>
		    </div>
		</div>
		...
		<div class="form-row">
		    <div class="form-group col-md-6">
		        ...
		        <input type="text" class="form-control" id="faxno" name="faxno" placeholder="대표 팩스번호" readonly>
		    </div>
		    
		    <div class="form-group col-md-6">
		        <label for="inputEmail4">사업장주소</label>
		        <input type="text" class="form-control" id="detailAdres" name="detailAdres" placeholder="주소" style="margin-bottom:8px;" readonly>
		        <input type="text" class="form-control" id="deptName8" name="deptName8" placeholder="상세주소" style="margin-bottom:8px;" readonly>
		        <input type="text" class="form-control" id="deptName9" name="deptName9" placeholder="상세주소2" readonly>  
		    </div>
		</div>
		...
		<div class="form-row">
		    ...
		    <div class="form-group col-md-6">
		        ...
		        <input type="text" class="form-control" id="dursName" name="dursName" placeholder="직책">
		    </div> 
		    <div class="form-group col-md-6">
		...
		<input type="hidden" id="cstmrNo" name="cstmrNo" value = ''>
		<input type="hidden" id="approveCode" name="approveCode" value = '0'>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">기업찾기</h5>
          ...
          <div class="modal-body">
			<div class="form-row">
			    <div class="form-group col-md-6">
			        <label for="inputEmail4">검색조건</label>
			        <select id="searchField" name="searchField">
			        	<option value="cstmrNm">사업명</option>  
			        	<option value="bizrno">사업자등록번호</option>
			        </select> 
			        <input type="text" class="form-control" id="searchText" name="searchText" placeholder="">
			        <button type="button" class="btn btn-primary" onClick="selectCompany()">검색</button>
			    </div>
			</div> 
		    <table class="table table-bordered table-sm">    
		        ...
		        <tbody id="selectCompanyTb">
			        <tr> 
			            <td><a href="#">이화학 주식</a></td>  
			            <td>223132-2323123</td>
			            <td>서울특별시 필동 사무소</td>
			        </tr> 
		        </tbody>
		    </table>
...
<!--             <button type="button" class="btn btn-primary" onClick="Household.fnUpdate()">modify</button>
            <button type="button" class="btn btn-secondary" data-dismiss="modal">cancel</button>
            <button type="button" class="btn btn-danger" onClick="Household.fnDelete()">delete</button> -->
          </div>
        </div>
      </div>
    </div>  
	...
	function insert() {
		...
		if($("#cstmrNo").val() == "") {
 		...
 		$.ajax({  
	        url: "/api/auth/insertCompanyManager",
	        type: "post",
	...
	function selectCompany() {
		...
		var formData = {};
		formData.searchField = $("#searchField").val();
		formData.searchText = $("#searchText").val();
 		$.ajax({
	        url: "/api/auth/selectCompanyList",
	        type: "post",
	        contentType: "application/json;charset=utf-8",  
	        dataType :'json',
	 		data : JSON.stringify(formData), 
	        success: function (map){  
	        	$("#selectCompanyTb").empty();
	        	...
	        	if ( companyList.length > 0 ) {
	        		for (var i=0; i<companyList.length ; i++ ) {
		        		...
		        		var txt ='' ;
		        		txt+='<tr>';
		        		txt+='    <td><a href="javascript:selectCompanyDetail(' + i + ')">' + companyList[i].cstmrNm + '</a></td>';
		        		txt+='    <td>' + companyList[i].bizrno + '</td>';
		        		txt+='    <td>' + companyList[i].detailAdres + '</td>';
		        	    txt+='</tr>';
	        			
	        			$("#selectCompanyTb").append(txt);
	        		}
	        	}
	        },
	        error: function(request, status, error){
	...
	function selectCompanyDetail(i){
		console.log(companyList[i]);
		$("#cstmrNm").val(companyList[i].cstmrNm);
		$("#bizrno").val(companyList[i].bizrno);
		$("#rprsntvNm").val(companyList[i].rprsntvNm);
		$("#telno").val(companyList[i].telno); 
		$("#faxno").val(companyList[i].faxno);  
		$("#detailAdres").val(companyList[i].detailAdres);  
		$("#cstmrNo").val(companyList[i].cstmrNo); 
