...
public class JavaGenProp {
	...
	private static boolean pbool(String key, boolean def){ return Boolean.parseBoolean(prop(key, String.valueOf(def))); }
	private static int     pint (String key, int def)    { return Integer.parseInt(prop(key, String.valueOf(def))); }
 ...
 // === Admin minimal add (no modes, always generate web + admin) ===
    private static final String ADM_PKG_UP = "adm";
    private static final String ADM_TEMPLATE_WEB_ROOT = "/adm/smp";                 // src/main/java/adm/smp/web/AdmTemplateController.java
    private static final String ADM_JSP_TEMPLATE_ROOT = "/WEB-INF/jsp/adm/smp/sample"; // /src/main/webapp/WEB-INF/jsp/adm/smp/sample/*.jsp
    private static final String ADM_JSP_TARGET_BASE   = "/WEB-INF/jsp/adm";            // 출력 루트
    ...
    private static String full_path= "/src/main/java";
	 ...
	 private static String service_name = prop("service.name", "Board");
	 private static String unitBizName  = prop("unit.biz", "basic");
	 private static String NaDa         = prop("author", "정지균");
	 private static String screenTitle  = prop("screen.title", "게시판");
    ...
    private static String server_ip= prop("db.url",    "jdbc:mysql://127.0.0.1:3306/nfi");
    private static String db_table = prop("db.table",  "board");
    private static String db_key   = prop("db.key",    "BOARD_IDX");
    /******************************************************************
    ...
    /** 관리자 JSP 연결 컨트롤러 생성 (adm.{bbs}.web.Adm{Service}Controller) */
    ...
    private static void createController() {
        try {
            ...
            String bizSeg  = getBizSeg(); // ex) hdd
            String bizPath = (isBlank(middle_pk_in_name) ? bizSeg : middle_pk_in_name).replace(".", "/"); // hdd/household
    ...
    private static void createSql() {
        try {
            ...
            getTableMap();
            List<String> cols = db_list; // 대문자/원형 컬럼명 리스트
            ...
            for (String key : cols) {
                ...
                if (isMySQLFamily() && isDateLike(U)) {
                    columnUpdate_.append("\t\t\t\t").append(U).append(" = ").append(mysqlDateParseExpr(camel)).append(",\n");
                } else {
    ...
    private static void createAdminJsp() {
        try {
            ...
            String bizSeg   = getBizSeg(); // ex) bbs
            String bizPath  = (isBlank(middle_pk_in_name) ? bizSeg : middle_pk_in_name).replace(".", "/"); // bbs/board
            ...
            if (fn.fileExists(tplList)) {
                ...
                if (!JSP_OVERWRITE && fn.fileExists(out)) System.out.println(out+" 존재. 생략");
                else { fn.writeText(out, fx.apply(fn.readText(tplList))); System.out.println("생성: "+out); }
            } else System.out.println("Admin JSP 템플릿 없음: " + tplList);

            if (fn.fileExists(tplMod)) {
                String out = targetDir + svcLower + "Modify.jsp";
                if (!JSP_OVERWRITE && fn.fileExists(out)) System.out.println(out+" 존재. 생략");
                else { fn.writeText(out, fx.apply(fn.readText(tplMod))); System.out.println("생성: "+out); }
            } else System.out.println("Admin JSP 템플릿 없음: " + tplMod);

            if (fn.fileExists(tplUnified)) {
                String out = targetDir + svcLower + ".jsp";
                if (!JSP_OVERWRITE && fn.fileExists(out)) System.out.println(out+" 존재. 생략");
                else { fn.writeText(out, fx.apply(fn.readText(tplUnified))); System.out.println("생성: "+out); }
            } else System.out.println("Admin JSP 템플릿 없음: " + tplUnified);
    ...
    private static HashMap getTableMap() {
        ...
        try{
            ...
            // MySQL: catalog = DB명; schemaPattern = null
            String catalog = null;
            try {
            ...
            while (rs.next()) {
                ...
                else if (typeName != null && typeName.toUpperCase().matches(".*(INT|NUMBER|LONG).*")) sTypeStr = "int";
    ...
    /** 
     * MySQL DATETIME 안전 파서
     * 허용: 2025-09-10 01:42, 2025-09-10T01:42, 2025-09-10 01:42:33, 2025-09-10T01:42:33.123, ...Z
     * 반환: STR_TO_DATE(정규화된 문자열, '%Y-%m-%d %H:%i:%s')
     */
    private static String mysqlDateParseExpr(String camelParamName) {
        String bind = "#{" + camelParamName + "}"; // MyBatis 바인딩
        // 1) 'T'→' '  2) 밀리초 제거  3) 'Z' 제거
        String norm = "REPLACE(REPLACE(SUBSTRING_INDEX(REPLACE(" + bind + ", 'T',' '), '.', 1),'Z',''),'T',' ')";
        // 4) 초가 없으면 ':00' 보강 (길이 16: 'YYYY-MM-DD HH:mm')
        String padSec = "CONCAT(" + norm + ", CASE WHEN LENGTH(" + norm + ")=16 THEN ':00' ELSE '' END)";
        // 빈문자열이면 NULL 처리
        return "STR_TO_DATE(NULLIF(" + padSec + ",''), '%Y-%m-%d %H:%i:%s')";
    }

    /** INSERT 전용: NULL/빈값이면 NOW()로 대체 */
