...
/**
 * @ClassName   : WebBoardPostController.java
...
public class WebBoardPostController {
    ...
    /** 목록 페이지 */
    @RequestMapping("/boardPost/boardPostList")
    public String list(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "brd/boardPost/boardPostList"; // 예) bbs/board/boardList.jsp
    }

    /** 등록/수정 페이지 */
    @RequestMapping("/boardPost/boardPostModify")
    public String modify(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "brd/boardPost/boardPostModify"; // 예) bbs/board/boardModify.jsp
    }
}
